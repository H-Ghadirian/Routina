...
extension HomeTCAView {
    ...
    func toggleCustomSuperSectionPause(_ sectionID: UUID) {
        guard
            let section = customTaskSections.first(where: {
    ...
    private func pauseCustomSuperSection(_ section: HomeCustomTaskSection) {
        ...
        guard
            let updatedSections = HomeCustomTaskSectionStorage.pausingSuperSection(
                section.id,
                taskIDs: taskIDs,
                at: pauseDate,
    ...
    private func resumeCustomSuperSection(_ section: HomeCustomTaskSection) {
        ...
        guard
            let updatedSections = HomeCustomTaskSectionStorage.resumingSuperSection(
                section.id,
                in: customTaskSections
            )
        else {
    ...
    func confirmCustomTaskSectionPrompt() {
        guard
            let result = HomeCustomTaskSectionStorage.upsertingSection(
    ...
    func applyCustomTaskSectionPrompt<Content: View>(to view: Content) -> some View {
        let title =
            if pendingCustomTaskSectionParentID != nil {
                "New Subsection"
    ...
    func applyCustomTaskSectionDeleteConfirmation<Content: View>(to view: Content) -> some View {
        view.alert("Delete Section?", isPresented: $isCustomTaskSectionDeleteConfirmationPresented) {
            ...
            Text(
                "Are you sure you want to delete \"\(pendingDeleteCustomTaskSectionTitle)\"? Tasks in this section will move back to their default sections."
            )
        }
    }
