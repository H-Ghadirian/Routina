...
extension HomeFeature {
    ...
    func handleMoveTodoToState(
        ...
        if newState == .done {
            guard !state.routineTasks[index].isCompletedOneOff,
                !state.routineTasks[index].isCanceledOneOff
            else { return .none }
        ...
        case .ready, .inProgress, .blocked:
            ...
            // SwiftFormat wraps this long capture list after the opening brace.
            // swiftlint:disable closure_parameter_position
            return .run {
                @MainActor [
                    id, rawValue = newState.rawValue, targetSectionKey, nextOrder, previousStateTitle, newStateTitle, timestamp = now
                ] _ in
                do {
                    let context = self.modelContext()
                    guard let task = try context.fetch(HomeTaskSupport.taskDescriptor(for: id)).first else { return }
    ...
    func handleCreateBacklogConfirmed(title: String, state: inout State) -> Effect<Action> {
        ...
        guard
            HomeBoardMutationSupport.createBacklog(
                title: title,
                now: now,
    ...
    func handleRenameSprint(id: UUID, title: String, state: inout State) -> Effect<Action> {
        ...
        guard
            HomeBoardMutationSupport.renameSprint(
                id: id,
                title: title,
    ...
    func handleAssignTodosToSprint(
        ...
        guard
            HomeBoardMutationSupport.assignTodosToSprint(
                taskIDs: taskIDs,
                sprintID: sprintID,
                data: &state.sprintBoardData
            )
    ...
    func handleSetBacklogRoutingTags(
        ...
        guard
            HomeBoardMutationSupport.setBacklogRoutingTags(
                backlogID: backlogID,
                tags: tags,
                data: &state.sprintBoardData
            )
