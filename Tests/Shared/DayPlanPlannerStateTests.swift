...
struct DayPlanPlannerStateTests {
    ...
    func displayModePickerReservesRoomForTimelineLabel() {
        ...
        #expect(
            DayPlanHeaderRangePickerVisibility.displayModePickerWidth
                > DayPlanHeaderRangePickerVisibility.compactDisplayModePickerWidth
        )
    }
    ...
    func calendarTaskViewModePickerReservesRoomForScheduleLabel() {
        ...
        #expect(
            DayPlanHeaderRangePickerVisibility.calendarTaskViewModePickerWidth
                > DayPlanHeaderRangePickerVisibility.compactCalendarTaskViewModePickerWidth
        )
    }

    @Test
    func compactRangePickerReservesRoomForItsCurrentChoice() {
        #expect(DayPlanHeaderRangePickerVisibility.compactVisibleRangeModePickerWidth >= 96)
        #expect(
            DayPlanHeaderRangePickerVisibility.visibleRangeModePickerWidth
                > DayPlanHeaderRangePickerVisibility.compactVisibleRangeModePickerWidth
        )
    }
    ...
    func preferredWeekModeAdaptsEffectiveRangeToAvailableWidth() throws {
        ...
        #expect(planner.availableVisibleRangeModes == [.day, .threeDays, .week])
        ...
        #expect(planner.visibleRangeMode == .threeDays)
        #expect(planner.visibleDates(calendar: calendar).count == 3)

        planner.setAdaptiveVisibleRangeMode(forAvailableWidth: 350, calendar: calendar, context: context)
    ...
    @Test
    func adaptiveRangeUsesTheSameMinimumDayWidthAsCalendarRendering() {
        ...
        #expect(
            DayPlanPlannerState.adaptiveVisibleRangeMode(
                forAvailableWidth: weekMinimumWidth
            ) == .week
        )
        #expect(
            DayPlanPlannerState.adaptiveVisibleRangeMode(
                forAvailableWidth: weekMinimumWidth - 1
            ) == .threeDays
        )
    }
    ...
    @Test
    func plannerHeaderUsesTheTighterVisibleWidthWhenParentWidthIsOptimistic() {
        #expect(
    ...
    @Test
    func regularPlannerCalendarCanCompressToTheReadableCompactDayWidth() {
        #expect(
    ...
    @Test
    func automaticPlannerSuggestionsIncludeAssumedDoneRoutine() throws {
        let calendar = gregorianCalendar
        let activityDate = try #require(date("2026-05-07T12:00:00Z"))
    ...
    @Test
    func automaticPlannerSuggestionsIncludeAssumedDoneChecklistRoutine() throws {
        let calendar = gregorianCalendar
        let activityDate = try #require(date("2026-06-22T12:00:00Z"))
        let referenceDate = try #require(date("2026-06-22T10:00:00Z"))
        let createdAt = try #require(date("2026-06-21T08:00:00Z"))
    ...
    @Test
    func automaticPlannerSuggestionsIncludeYesterdayAssumedDoneChecklistRoutine() throws {
        let calendar = gregorianCalendar
        let activityDate = try #require(date("2026-06-21T12:00:00Z"))
        let referenceDate = try #require(date("2026-06-22T10:00:00Z"))
        let createdAt = try #require(date("2026-06-21T08:00:00Z"))
    ...
    @Test
    func assumedDoneChecklistRoutineSurvivesTodayPlannerLayoutConflicts() throws {
        let calendar = gregorianCalendar
        let activityDate = try #require(date("2026-06-22T12:00:00Z"))
    ...
    @Test
    func resumedCountUpFocusSessionCreatesSeparatePlannerSegment() throws {
        let calendar = gregorianCalendar
        let context = makeInMemoryContext()
        let visibleDate = try #require(date("2026-05-07T12:00:00Z"))
        let startedAt = try #require(date("2026-05-07T09:30:00Z"))
        let pausedAt = try #require(date("2026-05-07T09:40:00Z"))
        let resumedAt = try #require(date("2026-05-07T10:05:00Z"))
        let now = try #require(date("2026-05-07T10:07:00Z"))
        let task = RoutineTask(
            id: UUID(),
            name: "Write notes",
            emoji: "📝",
        ...
        #expect(rawBlocks[0].startMinute == 9 * 60 + 30)
        #expect(rawBlocks[0].durationMinutes == 10)
        #expect(rawBlocks[0].titleSnapshot == "Write notes")
        #expect(rawBlocks[0].emojiSnapshot == "📝")
        #expect(rawBlocks[1].id != session.id)
        #expect(rawBlocks[1].startMinute == 10 * 60 + 5)
        #expect(rawBlocks[1].durationMinutes == 1)
        ...
        #expect(activeBlock.block.startMinute == 10 * 60 + 5)
        #expect(activeBlock.durationMinutes == 2)
    }

    @Test
    func finishedResumedCountUpFocusSessionKeepsSeparatePlannerSegments() throws {
        let calendar = gregorianCalendar
        let context = makeInMemoryContext()
        let startedAt = try #require(date("2026-05-07T09:30:00Z"))
        let pausedAt = try #require(date("2026-05-07T09:40:00Z"))
        let resumedAt = try #require(date("2026-05-07T10:05:00Z"))
        let endedAt = try #require(date("2026-05-07T10:12:00Z"))
        let task = RoutineTask(
            id: UUID(),
            name: "Write notes",
            scheduleMode: .fixedInterval
        ...
        #expect(blocks.map(\.startMinute) == [9 * 60 + 30, 10 * 60 + 5])
        #expect(blocks.map(\.durationMinutes) == [10, 7])
