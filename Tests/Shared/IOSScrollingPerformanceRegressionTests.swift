...
struct IOSScrollingPerformanceRegressionTests {
    ...
    func activeIOSSurfaceOwnsRefreshWorkAndStartupMaintenanceStaysOffMainActor() throws {
        ...
        let home = try Self.sourceFile("iOS/Screens/Home/HomeTCAView.swift")
        let homeTaskList = try Self.sourceFile("iOS/Screens/Home/HomeIOSTaskListView.swift")
        ...
        #expect(homeRefresh.contains("RoutinaIOSHomeScrollInteractionGate.isScrollActive"))
        #expect(homeRefresh.contains("scheduleDeferredRoutineUpdateRefreshRetry()"))
        #expect(homeRefresh.contains("refreshFileAttachmentTaskIDs()"))
        #expect(!home.contains(".onReceive(NotificationCenter.default.publisher(for: .routineDidUpdate))"))
        #expect(homeTaskList.contains("RoutinaIOSHomeScrollInteractionGate.recordScrollEvent()"))
    ...
    func homeDefersItsTagCatalogUntilTheTagPickerOpens() throws {
        ...
        #expect(picker.contains("@State private var selectedRuleByTagID"))
        #expect(picker.contains("displayedTagSummaries = selected + unselected"))
