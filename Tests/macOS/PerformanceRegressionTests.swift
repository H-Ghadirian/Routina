...
final class PerformanceRegressionTests: XCTestCase {
    ...
    func testMacHomeFocusToolbarUsesSingleTimerSlot() throws {
        ...
        XCTAssertTrue(source.contains("static let compactWidth: CGFloat = 540"))
        XCTAssertTrue(source.contains("static let focusedWidth: CGFloat = 1040"))
        XCTAssertTrue(source.contains("static let activeHostWidth: CGFloat = focusedWidth"))
        XCTAssertTrue(source.contains("static let hostReleaseDelay: TimeInterval = 0.26"))
        XCTAssertTrue(source.contains("static let height: CGFloat = 44"))
        XCTAssertTrue(source.contains("@State private var isFocused = false"))
        XCTAssertTrue(source.contains("@State private var keepsActiveHost = false"))
        XCTAssertTrue(source.contains("@State private var hostTransitionID = 0"))
        XCTAssertTrue(source.contains("@State private var focusRequestID = 0"))
        XCTAssertTrue(source.contains("width: hostWidth"))
        XCTAssertTrue(source.contains(".frame(width: targetWidth, height: HomeMacToolbarSearchLayout.height)"))
        XCTAssertTrue(source.contains("(isFocused || keepsActiveHost) ? HomeMacToolbarSearchLayout.activeHostWidth : HomeMacToolbarSearchLayout.compactWidth"))
        XCTAssertTrue(source.contains(".onChange(of: isFocused)"))
        XCTAssertTrue(source.contains("DispatchQueue.main.asyncAfter(deadline: .now() + HomeMacToolbarSearchLayout.hostReleaseDelay)"))
        ...
        XCTAssertTrue(source.contains("isFocused ? HomeMacToolbarSearchLayout.focusedWidth : HomeMacToolbarSearchLayout.compactWidth"))
        ...
        XCTAssertTrue(source.contains("NSTextField(string: text)"))
        ...
        XCTAssertTrue(source.contains("isFocused && (isCreatingTask || canCreateTaskFromQuery)"))
        XCTAssertTrue(source.contains("controlTextDidBeginEditing"))
        XCTAssertTrue(source.contains("@Binding var isFocused: Bool"))
        XCTAssertTrue(source.contains("@State private var focusDismissRequestID = 0"))
        ...
        XCTAssertTrue(source.contains("dismissSearchIfClickIsOutsideVisiblePill"))
        XCTAssertTrue(source.contains("!view.bounds.insetBy(dx: -2, dy: -2).contains(clickLocation)"))
