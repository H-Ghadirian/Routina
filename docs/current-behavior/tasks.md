...
- Tracking entries are analysis entries for logging what happened and how time was spent. Task forms split the primary kind into `Tracking` / `Task`, and show `Todo` / `Routine` only inside `Task`. Tracking can use routine-like metadata including `Completion`, duration, all-day/time availability, interval/calendar repeat, checklist item runout, steps for standard tracking entries, checklist items for checklist tracking entries, estimates, actual duration, focus metadata, notes, links, tags, goals, places, media, attachments, relationships, events, comments, search, Stats, Timeline, heatmaps, and archive state. Tracking schedule behavior is always Gentle and the `Due Style` picker is hidden. Tracking does not expose or persist due dates, reminders, or todo date availability; non-daily tracking can store planned dates, while daily tracking follows daily routine planning rules.
