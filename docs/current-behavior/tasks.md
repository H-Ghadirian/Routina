...
- Daily non-Tracking routines already belong to the daily routine area and do not expose stored planned-date controls; daily Tracking can still be explicitly planned when cadence is enabled.
...
- On Mac, row context menus can create named custom top-level task sections and move active unpinned tasks or tracking rows into them. Settings -> Sections can also create, rename, delete, and configure custom sections. Custom sections use durable section IDs, their own manual-order keys, and the same full-bleed top-level sidebar surface style as `Today`, `Tracking`, and `Future`. Assigned rows are claimed into their custom section before built-in planning, tracking, and future sections can claim them, while pinned and archived rows keep their existing priority. Settings-managed rules can additionally claim unassigned active unpinned rows with explicit planned dates for today, explicit planned dates for tomorrow, Tracking task type, or any configured section tag into a custom section before the built-in fallback sections; tag rule matching uses Routina's case- and accent-insensitive tag identity. Manual custom assignments stay stronger than rule matches. Planning a custom-section row with `Plan to do` clears the custom assignment so it can move back into `Today` or `Tomorrow`. Right-clicking a custom section header exposes `Rename Section` and `Delete Section`; renaming updates only the section title, while confirming deletion removes only the section, clears matching custom-section assignments/manual ordering, and leaves the tasks themselves intact.
