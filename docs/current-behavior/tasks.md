...
- Fixed date-based structured schedules that produce at most one occurrence per day can compose with an Available window or Time block. Recurrence selects the valid dates; the range start opens each occurrence and the range end closes it. Home, Task Detail, notifications, missed resolution, and Planner use those same boundaries. Hourly and multiple-times-per-day schedules reject one outer window explicitly because their occurrence identity cannot be represented safely by one day-level range.
