...
- Add Task, Edit Task, and the shared form model expose their current recurrence as one lossless draft spanning cadence-free, item-runout, after-completion, compact calendar, and fixed-anchor structured schedules. The visible Simple / Advanced selector remains during the staged unified-composer transition. Draft translation preserves exact-time and time-window availability for compact rules and explicitly rejects structured combinations the current runtime cannot encode instead of dropping fields.
