...
- Existing Tracking entries keep the internal `record` task type for compatibility and remain editable with their routine-like analysis metadata, actual duration, dedicated Home/Stats placement, and implicit Gentle behavior. Full task forms no longer create new Tracking records, and no automatic migration rewrites their type or history.
...
- Planned dates are date-only Home-list planning hints for todos, cadence-enabled Tracking entries, and non-daily routines. They are not availability, deadline, reminder, completion history, or stored Planner blocks. Planner day agendas surface active date-only planned tasks for the selected day.
