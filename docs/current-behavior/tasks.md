...
- Simple recurrence remains the default and keeps the existing Interval / Calendar controls. On iOS and macOS, Add Task and Edit Task use shared multi-select controls for calendar weekdays and monthly dates, preserving every selection. Dates 29–31 have an adaptive-date indicator and explain that shorter months use their last valid day. Interval repeats own Simple's single daily-repeat path.
- Routine and cadence-enabled Tracking forms can instead select Advanced recurrence for every-N hourly, daily, weekly, monthly, or yearly schedules, including daily multiple times, selected weekdays, multiple month-day dates or ordinal-weekday patterns, yearly month/day patterns, start dates, end dates/counts, and hourly daily windows. If several adaptive monthly dates resolve to the same shorter-month date, Routina produces one occurrence for that timestamp.
- Add Task, Edit Task, and the shared form model expose their current recurrence as one lossless draft spanning cadence-free, item-runout, after-completion, compact calendar, and fixed-anchor structured schedules. Add and Edit saves resolve an explicitly edited draft directly; while the visible Simple / Advanced controls remain during the staged transition, changing one of those legacy controls deliberately returns ownership to the legacy form fields. Draft translation preserves exact-time and time-window availability for compact rules, including cadence-free one-time tasks, and explicitly rejects structured combinations the current runtime cannot encode instead of dropping fields.
