...
- iOS development builds retain `CLLocationManager` access for Places. Production compiles a no-op location client instead, so it cannot request or read device location while Places remains unavailable.
