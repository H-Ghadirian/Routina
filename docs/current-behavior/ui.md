...
- The Mac full task-detail toolbar renders its task actions as one explicitly spaced control cluster rather than a native toolbar group, so controls do not visually merge. `Done` is the only prominent task action, using a fixed-height custom rounded-rectangle label so its short text does not collapse into a circle. Routine Pause/Resume and one-off `Cancel todo` use the same fixed control height as secondary text actions; `Cancel todo` uses a non-close icon instead of an `xmark.circle` near the close control. Link, sharing, edit, minimize, and close render as equal-size icon toolbar controls, with close remaining the single rightmost `xmark` for leaving full Details.
