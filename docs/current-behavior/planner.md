...
- Planner has one canonical header date/range control in the right utility cluster. It shows the selected day in Day mode, shows the visible week range in Week mode, and opens date selection in the right Planner sidebar when pressed. Show today appears only when today's column is outside the visible calendar; previous/next, Day/Week, and Day spacing controls stay grouped as navigation/view controls on the left.
