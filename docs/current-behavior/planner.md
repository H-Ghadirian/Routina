...
- Planner can derive timeline task activity for day agenda and Calendar `List` review, but the editable Calendar `Schedule` only renders task-backed blocks that come from explicit Planner placements, all-day task placements, exact `At time` / exact date-time task metadata, or fixed `Time block` ranges. `Available window` timing is availability metadata and does not create a default timed Schedule block. Recorded completions, missed/canceled day outcomes, and synthetic assumed-done activity hide task-backed timed and all-day Schedule blocks for that day.
