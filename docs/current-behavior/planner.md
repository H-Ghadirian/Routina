...
- Planner can derive timeline task activity for day agenda and Calendar `List` review, but the editable Calendar `Schedule` only renders task-backed blocks that come from explicit Planner placements, all-day task placements, or exact `At time` / exact date-time task metadata. `Window` timing is availability metadata and does not create a default timed Schedule block. Recorded completions, missed/canceled day outcomes, and synthetic assumed-done activity hide task-backed timed and all-day Schedule blocks for that day.
...
- Exact-time routines and one-off tasks with exact availability/deadline times can create timed Planner blocks at their scheduled time. Time-window routines and one-off availability windows do not create default timed Planner blocks; Planner removes stale auto-generated window blocks that still exactly match the old window-start placement while keeping manually moved or resized blocks for that task.
