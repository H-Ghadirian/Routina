...
- Opening a task while Planner is active, whether from the left task list or from Planner itself, keeps Planner visible and shows task details in a companion pane on the right side of the Mac detail area when the detail area can fit the fixed companion pane plus a Day-capable Planner surface. At tight widths, the Planner calendar can adapt down to a compact Day layout with a narrower day column to make room for the pane. The pane has close and fullscreen controls; fullscreen switches to the regular Details surface for the selected task. Full Details opened from that control can minimize back to the previous companion-pane layout, while its separate close control returns to Planner and clears the pane. When a recorded Done row is opened from Calendar `List`, the companion pane carries that row's exact day and completion identity and shows `Done this day` controls for the work's start time and actual duration. Duration accepts custom whole-minute Hours and Minutes as well as presets and incremental adjustment, constrained so the work finishes within the selected day. Save updates only that completion record, deriving its finish timestamp from start plus duration; it does not mutate Planner blocks, planned rows, recurrence, availability, reminders, estimates, other occurrences, or other days. Planned and assumed-done rows open normal task details without this completion card. The task title in the Planner-adjacent detail pane, and in Full Details expanded from that pane, can be dragged into the editable Schedule grid or all-day lane using the same task payload as left-sidebar task rows while keeping the title copy menu. Planner's own right sidebar remains reserved for slot actions, day agendas, and date selection. Only one right-side secondary surface can be open in the Mac detail area: opening task details closes Planner's internal right sidebar, and opening Planner's internal right sidebar closes task details. Double-clicking a Mac task-list row opens the selected task in the full Details surface instead of the companion pane.
