...
- [0589](../decisions/0589-bound-manual-icloud-refresh.md)
...
- iCloud sync, reset, backup import, and backup export live in one iCloud & Backup settings section. `Sync Now` verifies the manual iCloud download only; local uploads remain asynchronous and must not be reported as completed until CloudKit records a successful export. The direct pull stops after 60 seconds rather than leaving `Checking iCloud for updates...` indefinitely. A failure states that existing local data is safe and explains whether to check the network, iCloud sign-in, or temporary service availability before using the re-enabled `Sync Now` action again.
