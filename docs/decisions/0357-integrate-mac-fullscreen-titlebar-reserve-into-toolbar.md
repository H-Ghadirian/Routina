...
When Home is fullscreen, it must not add a separate fullscreen-only vertical reserve above the SwiftUI toolbar. The existing Home top toolbar row is the stable chrome row for the fullscreen session. Its leading traffic-light-safe region remains empty through toolbar leading padding, and Routina controls start after that native traffic-light region.
