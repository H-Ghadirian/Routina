...
- **[0179](0179-make-all-day-an-availability-choice.md) refines [0093](0093-support-all-day-routines.md) and [0178](0178-make-recurrence-availability-independent.md) for task form scheduling controls:** All-day is presented as a timing choice inside deadline or routine Availability controls, not as a separate top-level checkbox that can conflict with exact time or window settings.
- **[0178](0178-make-recurrence-availability-independent.md) refines [0009](0009-support-routine-time-ranges.md) and [0177](0177-separate-interval-and-calendar-repeat-controls.md) for recurrence availability:** Routine forms show `Availability` as an independent section for interval and calendar repeats; interval recurrence rules may store exact time or time ranges while the interval still determines the due day.
...
- **Tasks, routines, and scheduling:** Recurrence lives in typed SwiftData columns, with legacy JSON only as migration input. Forms distinguish interval repeats from calendar repeats before asking for daily, weekday, or month-day pattern details, and present Availability as an independent any-time/all-day/exact-time/window choice for both interval and calendar repeats. Routines store Due/Gentle scheduling and Standard/Checklist/Runout format separately, but forms present Standard/Checklist finish with Runout nested under checklist cadence/timing. Exact-time and time-range routines resolve missed assumptions instead of staying overdue forever. Optional checklists can attach to ordinary tasks and block manual completion until every item is done. All-day intent is first-class task data for todos and routines and is mutually exclusive with recurrence exact-time/window availability in forms and save builders. Device battery charge routines are opt-in and removed while their setting is disabled.
...
| [0179](0179-make-all-day-an-availability-choice.md) | Make All Day an Availability Choice | Accepted | 2026-06-07 |
