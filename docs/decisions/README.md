...
- **Focus timer pause:** Task and unassigned `FocusSession` timers can be paused and resumed while remaining the active focus session. Elapsed time, remaining time, focus blocks, count-up planner duration, widgets, Live Activities, Watch payloads, stats, and achievements count active focused time instead of paused wall-clock time. Finishing or abandoning a paused timer closes the pause first.
