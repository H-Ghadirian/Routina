...
- **Stats dashboard adaptive width:** Stats dashboard content uses the available detail-column width on iOS and macOS instead of a fixed narrow max width. Dashboard sections rely on adaptive grids and cards to add columns on wide windows, reducing vertical scroll while preserving compact behavior.
...
- **Planner board focus:** Sprint board focus sessions render in Planner from `SprintFocusSessionRecord`. Active and finished unallocated board focus appears as board focus time, task allocations render as task-colored focus blocks for their allocated minutes, and any remaining unallocated duration stays visible as board focus. These derived blocks contribute blocked intervals but remain projections of board focus history rather than independent manual planner blocks.
- **Focus timeline entries:** Timeline shows task, unassigned, and board focus sessions as first-class Focus entries at their start time. Active, paused, and completed task focus sessions can appear as Timeline evidence; abandoned task focus sessions stay out of Timeline so canceled focus does not look like accepted focus time. Board focus sessions render through sprint focus history, and abandoning active board focus removes that history record. Focus entries are visible in All and Focus filters, can inherit linked-task tags and importance/urgency, and remain separate from media, done, missed, and canceled outcome filters.
...
- **Focus chart details and grouping:** Stats focus duration points include task, unassigned, and board focus contributions. The Focus time chart can group by day, week, or month for the selected range, and hover or selection reveals exact focused duration plus the top focus contributors for that bucket. Completed task and unassigned focus sessions bucket by completion day before optional grouping, completed board focus sessions bucket by stop day, and active focus sessions contribute live current-day focused duration. Stats screens refresh periodically only while unpaused focus is active so current-day totals do not go stale.
...
- **Focus timer pause:** Task, unassigned, and board focus timers can be paused and resumed while remaining the active focus session. Elapsed time, remaining time, focus blocks, count-up planner duration, widgets, Live Activities, Watch payloads, stats, achievements, and board focus history count active focused time instead of paused wall-clock time. Finishing a paused timer closes the pause first; abandoning an active board focus timer removes it from board focus history.
- **Today focus widget:** macOS includes a Today Focus widget that shows the current day's total task, unassigned, and board focus time from the shared widget stats payload. Focus contributes the portion overlapping the reference day, capped by each session's persisted active duration, and unpaused active focus increments from the last widget refresh.
- **Sleep stats scope:** Stats includes a top-level Sleep dashboard scope on iOS and macOS. The scope shows range-bound sleep summary cards derived from `SleepSession` history, with active sleep sessions contributing duration up to the current reference date. Sleep achievements stay inside the Achievements section category picker.
- **Recent Wins stats scope:** Stats includes a top-level Wins scope on iOS and macOS. The scope shows Recent Wins cards for today, this week, this month, and this year, including only periods with recorded accomplishments, and has an empty state when there are no current accomplishments.
- **Stats achievements:** Stats includes an Achievements dashboard scope and all-time achievement badges derived from completed focus sessions, completed sleep sessions, finished Away sessions, planned-duration Away completions, completed `RoutineLog` history, emotion logs, saved places and finished check-ins, goals, and standalone notes. Badges preserve the focus milestones for total focus time, five-minute blocks, single-session depth, strong focus days, streaks, rolling-week consistency, and returning after quiet days while adding sleep, away, done, emotion, place, goal, and note milestones. Locked badges remain visible with progress. The Achievements section can be viewed by All, Focus, Sleep, Away, Done, Emotions, Places, Goals, and Notes categories. In Progress and Achieved appear as separate status tabs; Achieved adds Today, This Week, This Month, and This Year period tabs derived by comparing current earned badges with pre-period history. Done badges include a longer ladder for total completions, high-output days, streaks, and full-week consistency.
- **Focus shields:** iOS Focus can optionally block user-selected apps, categories, and websites during active focus sessions using FamilyControls and ManagedSettings. Routina stores only opaque Screen Time tokens, applies shields while any unpaused `FocusSession` or active `AwaySession` is active, suspends them while the focus timer is paused, and clears them when no protected session remains. macOS uses a separate best-effort blocker that defaults on and closes user-selected apps while an unpaused task `FocusSession` or active `AwaySession` is active; users can disable it or change the app list. Native macOS website blocking is deferred until there is a supported entitlement-backed implementation. Focus timers and Away sessions continue to work if shielding is unavailable or denied.
- **Away sessions:** Away sessions are app-level protected timers for off-phone periods such as waking up, reset breaks, outside time, meals, and wind-down. They are stored as `AwaySession` rows, do not overlap with Sleep or Focus timers, reuse the existing shield/blocker selection while active, render as protected planner blocks, and contribute to dedicated Away stats rather than Focus metrics. They can use a fixed planned duration or count up with `plannedDurationSeconds == 0`; count-up Away has no planned end, does not auto-complete through expiry checks, and finishes at the actual user-ended timestamp.
...
- **Task links:** Tasks store an ordered list of sanitized HTTP/HTTPS links in addition to the legacy first-link field. Add/edit forms present repeatable link rows, task details render every link, and backup/import, CloudKit direct pull repair, task sharing, iCloud usage estimates, and AI task summaries preserve the full list while older single-link payloads remain supported.
...
- **Optional task checklists:** Checklist items can attach to any task type. Checklist and runout routine formats keep their schedule/completion semantics, while checklists on standard routines and one-off todos are optional progress items that do not complete the task by themselves or change scheduling. Manual completion is blocked until every optional checklist item is checked. Standard routine optional checklist progress resets when the routine completes; todo checklist state stays with the todo.
- **Standalone notes:** Standalone notes are SwiftData `RoutineNote` records with optional title, body, image, voice data, shared normalized tags, and linked `RoutineNoteAttachment` file records. Home add controls can create notes, and note detail surfaces can edit existing notes while preserving `createdAt` as the timeline date and using `updatedAt` for an edited-date marker. Timeline shows notes as first-class entries under a Notes filter. Timeline tag filtering, Settings tag management, related-tag learning, backup/import, reset, duplicate cleanup, and iCloud usage estimates include note data, note media, and note tags.
...
- **Emotion logs:** Emotion logs are standalone SwiftData `EmotionLog` records with pleasantness and energy values, one or more emotion families/details, intensity, optional body areas/reflection, and optional links to notes, goals, tasks, places, and sleep sessions. Emotion capture uses explicit Pleasantness and Energy segmented pickers instead of a chart-based mood map, suggested emotion families stay inside the selected pleasant/unpleasant and low/high energy quadrant, and a Tired / need sleep quick log preselects the low-energy tired state while linking the latest sleep session when available. Context link tiles open bounded searchable pickers instead of large menus. iOS Home actions and the macOS Home add menu can create emotion logs. On macOS, Add Emotion opens in the Home detail area and saving routes to the saved Timeline emotion entry. Emotion detail views can edit the existing log in place without creating a replacement record and can open linked notes, goals, tasks, and sleep sessions through app deep-link routing. Timeline shows emotions under an Emotions filter and does not generate insight summaries.
...
- **Mac note creation and editing:** Mac Home presents Add Note and Edit Note in the main detail area instead of a sheet, matching the main-surface creation and editing model used by the primary Home workflows. Shared note detail keeps a modal edit fallback for hosts that do not provide an inline edit route.
...
- **Timeline row details:** Timeline note rows open note detail, place check-in rows open a dedicated place check-in detail view, and sleep rows open the linked sleep session in Home Planner on macOS by scrolling to and highlighting its protected block. Place detail is read-oriented; correction/editing remains in the existing Places/check-in history surfaces.
...
| [0122](0122-show-focus-achievement-badges.md) | Show Focus Achievement Badges | Superseded | 2026-05-31 |
...
| [0126](0126-show-focus-sessions-in-timeline.md) | Show Focus Sessions in Timeline | Superseded | 2026-06-01 |
...
| [0132](0132-categorize-achievement-badges.md) | Categorize Achievement Badges | Superseded | 2026-06-02 |
...
| [0141](0141-show-achievement-period-celebrations.md) | Show Achievement Period Celebrations | Superseded | 2026-06-03 |
