...
- **Emotion logs:** Emotion logs are standalone SwiftData `EmotionLog` records with pleasantness and energy values, emotion family/detail, intensity, optional body areas/reflection, and optional links to notes, goals, tasks, places, and sleep sessions. Emotion capture uses explicit Pleasantness and Energy segmented pickers instead of a chart-based mood map. iOS Home actions and the macOS Home add menu can create emotion logs. On macOS, Add Emotion opens in the Home detail area and saving routes to the saved Timeline emotion entry. Timeline shows emotions under an Emotions filter and does not generate insight summaries.
