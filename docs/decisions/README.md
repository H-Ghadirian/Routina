...
- **Emotion logs:** Emotion logs are standalone SwiftData `EmotionLog` records with mood-map coordinates, emotion family/detail, intensity, optional body areas/reflection, and optional links to notes, goals, tasks, places, and sleep sessions. iOS Home actions and the macOS Home add menu can create emotion logs. Timeline shows them under an Emotions filter and does not generate insight summaries.
...
- **Mac Home add menu:** The Mac Home sidebar `+` opens a menu with Emotion, Note, Goal, Task, Check In, and Sleep actions. Emotion opens the standalone emotion logger; Note opens the standalone note editor; Goal switches to Goals and opens the inline goal editor; Task opens the existing add-task form; Check In opens the existing Places/check-in workspace; Sleep starts the existing sleep-mode flow. Goals mode does not add a separate toolbar `+`.
