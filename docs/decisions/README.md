...
- **Emotion trends:** Stats includes an Emotion Trends chart that plots daily average pleasantness and energy from emotion logs in the selected range, with highest average intensity shown as a factual highlight. Days without emotion logs are omitted from the line series.
...
- **Stats dashboard customization:** Stats dashboard edit mode supports hiding, restoring, and dragging visible dashboard sections to reorder them on iOS and macOS. Range/filter controls stay fixed above the dashboard, and reset clears both hidden sections and custom ordering.
