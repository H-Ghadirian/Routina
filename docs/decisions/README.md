...
- **Home task row visibility:** Users can choose which Home task row fields are visible from Settings > Appearance > Task Row. The default is the full row, and the preference stores hidden fields so newly added row fields appear by default.
