...
- **Focus shields:** iOS Focus can optionally block user-selected apps, categories, and websites during active focus sessions using FamilyControls and ManagedSettings. Routina stores only opaque Screen Time tokens, applies shields while any unpaused `FocusSession` is active, suspends them while the focus timer is paused, and clears them when active focus ends. macOS uses a separate best-effort blocker that defaults on and closes user-selected apps while an unpaused task `FocusSession` is active; users can disable it or change the app list. Native macOS website blocking is deferred until there is a supported entitlement-backed implementation. Focus timers continue to work if shielding is unavailable or denied.
