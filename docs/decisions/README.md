...
- **Focus, Away, Sleep, and shielding:** Sleep, Focus, and Away are app-level protected session types that must not overlap. Task, unassigned, and board focus can pause/resume, count active time instead of paused wall-clock time, and feed planner, timeline, stats, widgets, Live Activities, Watch payloads, and achievements. Away uses dedicated stats, can be fixed-duration or count-up, and links overlapping completed activity into the Away planner block instead of showing duplicate planner cards. Blocking is configured in one Settings section with Focus, Away, and Sleep applicability. iOS shields and the macOS app blocker apply while an enabled protected mode is active, then clear when protection ends.
