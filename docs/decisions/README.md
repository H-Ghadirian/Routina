...
- **Task and done media filtering:** Home task filters and Timeline/Done filters share a media filter with All, Any Media, Image, and File options. Task images come from `RoutineTask`, file presence comes from linked `RoutineAttachment` task IDs, and non-task timeline entries are hidden when a media filter is active.
