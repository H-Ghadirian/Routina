...
- **Focus 2048 board:** Stats includes a read-only Focus 2048 section that converts each full two focused hours into a base `2` tile and merges those tiles into power-of-two focused-hour tiles. Partial progress toward the next two-hour tile is shown separately, and the section derives from the same filtered focus total as other focus stats.
