...
- **Tasks, routines, and scheduling:** Recurrence lives in typed SwiftData columns, with legacy JSON only as migration input. Forms present routine Availability as an independent any-time/all-day/exact-time/window choice before asking whether the due day repeats by interval or calendar pattern. Routines store Due/Gentle scheduling and Standard/Checklist/Runout format separately, but forms present Standard/Checklist finish with Runout nested under checklist cadence/timing. Exact-time and time-range routines resolve missed assumptions instead of staying overdue forever. Optional checklists can attach to ordinary tasks and block manual completion until every item is done. All-day intent is first-class task data for todos and routines; routine all-day is mutually exclusive with recurrence exact-time/window availability in forms and save builders, while todo all-day stays independent from deadline controls. Device battery charge routines are opt-in and removed while their setting is disabled.
