...
**Desired experience:** The person keeps stable Base values between occurrences and can choose either an on-due-date change or a gradual increase over a visible lead window for Importance, Urgency, and/or Pressure while creating or editing a repeating task. The four Task Ladder values stay together, and Changes over time sits directly beneath them rather than hiding in an optional-detail catalog; one-time tasks do not mention this inapplicable feature. If a repeating task's rule is unavailable, the form names the concrete Behavior & Schedule choice needed. On Mac, each supported metric reads directly as its Base value leading to either `No change` or a higher target, without enable checkboxes. The first target reveals the one shared timing choice, Gradually reveals its lead days, and one live sentence summarizes what will change before a quiet completion-reset note. Returning every metric to `No change` turns the rule off. iOS retains its more detailed lifecycle preview. Task Details keeps the four current values directly editable, shows a configured rule summary without adding another setup surface, and Task Ladder's read-only Now view explains an adjusted task without silently rewriting Base. On Mac, the current choices remain easy to scan without simultaneously showing every alternative; compact values sit together with small gaps, selecting one current value horizontally reveals only that complete picker and pushes later values right, and selecting an option returns it to the compact state.
...
**Evidence:** User-described need on 2026-08-16; clarified on 2026-08-18 that the resulting values must be understandable from Task Details, Edit Task, Add Task, and Task Ladder; and refined on 2026-08-23 to keep setup in Add/Edit, avoid crowding Task Details, expose exact repeating-task eligibility, omit the feature for one-time work, progressively reveal Mac value alternatives, and preview after-completion, gradual, due-date, and overdue states. The selected curves, lead-window control, affected dimensions, and Base/Now mental model remain product assumptions to validate through use.
...
**Current limitation:** Mac Backlog has no unsectioned explicit destination. A task move menu can create-and-assign a new section, while Settings is required when the person wants to prepare an empty catalog. The task form's `Path` menu offers `Backlog › <section>` after a section exists, and the beta Board's separately named backlog can still add ambiguity.
...
**Task configuration refinement:** Add and Edit use a predictable three-part core: Behavior & Schedule for what the task does over time, Task Ladder values for the four independent judgments and their optional changes over time, and Organization for Path, Tags, Flags, and Task Ladder grouping. The person does not have to search several distant cards for related values, and Task Details can remain a compact review-and-action surface.
