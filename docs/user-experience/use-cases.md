...
On Mac, Backlog can be narrowed without reorganizing it. Its independent filter surface covers task type and status, created date, current Importance/Urgency/Pressure, Thinking needed, estimate and media presence, Tags, and Flags; it deliberately omits Planner layers, Timeline outcomes, main-task-list appearance/visibility, grouping, and sorting. Filtering hides empty super sections and subsections so every visible branch explains at least one matching task. It does not move a task or change its path, and a filtered-out search match still prevents accidental duplicate creation.
