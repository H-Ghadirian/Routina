...
**Desired experience:** Each record keeps its own meaning and appears in a coherent personal history. Optional context features do not complicate the core task experience when unavailable or disabled, including by leaving unavailable choices in creation, history filters, Timeline rows, Stats reports, task relationships, deep links, Settings language, or notifications. Preserved records remain stored for later re-enablement without advertising a feature that is absent from production. On iOS, disabling Sleep or its shake shortcut means a physical shake cannot open or complete a Sleep-mode start, and preserved Sleep sessions do not remain visible in Timeline while Sleep is unavailable. The first iPhone release similarly omits Routina's standalone Event feature everywhere; Apple Calendar review remains available because it imports external calendar entries as tasks rather than creating Event records.
...
**Evidence:** User-provided first-release iOS Timeline feedback on 2026-08-31 established that standalone Event is disabled by default and unavailable to users, so preserved or fixture-generated Events must not appear in release-facing iOS surfaces.
