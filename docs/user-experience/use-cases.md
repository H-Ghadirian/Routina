...
On Mac, `All` offers every Task Ladder value as an independent cross-surface filter and uses the current values the person sees in Task Ladder. Its Tags card stays calm when the catalog is large: it shows only active rules until the person deliberately opens searchable browse, where selected tags stay pinned and suggestions remain bounded. Stats uses that same compact Tags interaction instead of repeating the full catalog as include, suggestion, and exclude chip clouds.
...
**Evidence:** User-requested Mac filter redesign and Stats follow-up on 2026-08-24.
...
**Evidence:** User-reported Focus, time, and estimate confusion and explicit correction request on 2026-08-24.
