...
On Mac, the workspace-level filter action sits immediately left of the combined workspace menu and appears only for Planner and Backlog. The menu remains fixed at the trailing edge when the action disappears in other workspaces. Planner keeps its established Shared, Task List, Timeline, and Calendar meanings, while Backlog titles its surface `Filter and Sort` and offers only controls relevant to its own task collection: due-date order, type/status, created date, current Task Ladder values, estimate, media, Tags, and Flags. The active-workspace icon makes a non-default filter or Backlog order discoverable without adding irrelevant chrome to Task Ladder, Stats, or other workspaces.
...
On Mac, Backlog can be narrowed or ordered without reorganizing it. Its independent `Filter and Sort` surface covers task type and status, created date, current Importance/Urgency/Pressure, Thinking needed, estimate and media presence, Tags, Flags, and Default/Due Soonest/Due Latest order. Due sorting uses the task's real due boundary—its one-time deadline or a repeating Due routine's next occurrence—and leaves undated or Gentle work after dated work in either direction. It changes row order within each existing Backlog group, not section order or task placement; sorting alone also keeps empty destinations reachable. Filtering still hides empty super sections and subsections so every visible branch explains at least one matching task. Neither operation moves a task or changes its path, and a filtered-out search match still prevents accidental duplicate creation.
