...
**Example:** Searching `hide` returns `Flags` with four hide behaviors on Mac and the three iOS-relevant hide behaviors on iPhone or iPad. Searching `Planner calendar`, `Calendar list`, or `keyboard` finds the relevant Mac destination but does not create a false iOS result. Searching `backup` can return `iCloud & Backup` with its sync, export, import, or backup concepts.
...
**Availability:** Production for core reports; optional reports vary by platform, data, and feature gate. Apple Health is unavailable in the first iOS and iPadOS release.
