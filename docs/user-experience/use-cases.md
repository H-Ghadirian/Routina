...
**Desired experience:** The person keeps stable Base values between occurrences and can choose either an on-due-date change or a gradual increase over a visible lead window for Importance, Urgency, and/or Pressure while creating or editing the task. The four Task Ladder values stay together, and Changes over time sits directly beneath them rather than hiding in an optional-detail catalog. If the rule is unavailable, the form names the concrete Behavior & Schedule choice needed. Before saving, the person can see the next occurrence after completion at Base, values during the lead window, targets on the due date, and the values that persist after the due date until completion. Task Details keeps the four current values directly editable, shows a configured rule summary without adding another setup surface, and Task Ladder's read-only Now view explains an adjusted task without silently rewriting Base.
