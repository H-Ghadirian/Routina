...
**Example:** A task with a titled reference link and an explanatory screenshot shows `LINKS` and `IMAGE` together in one content card; a weekly routine can still become biweekly without losing its earlier weekly history.

**Evidence:** User-described need and interaction preferences on 2026-08-21, including direct feedback that duplicate Details sections and separated link/image content were confusing.
