...
**iOS global access:** New is a predictable two-choice surface from every primary tab: `Create Task` is always first and opens the existing fast task flow; `Focus` is always second. Experimental capture types do not displace or reorder those actions.

**Desired experience:** The shortest path asks for a meaningful title and confirms the save. On a genuinely new iOS installation that has not observed a task, loaded Home asks what the person would like to get done and offers one direct `Create Your First Task` action. It withholds task-type controls, Filters, Backlog, Timeline, and Task Ladder until a first task is created, imported, restored, or synchronized, while Settings and New remain available. Once any task appears, first-use guidance never returns merely because the catalog later becomes empty. Common natural-language dates and times are recognized when typed with the title, including a weekday plus day and month followed by a 24-hour time. An unqualified one-off date/time means when the task is available, while `due` or `by` explicitly means a deadline. On Mac, the workspace-labeled toolbar menu begins with `Add New Task` beside Focus and shows Control-Option-Command-T for the direct keyboard route, so capture stays discoverable in the same list as Planner, Backlog, Task Ladder, and the other destinations. On iOS, recognized one-off availability is visible as an `Available` detail before saving so the person can verify the date and time. Exact availability offers an optional reminder choice before saving and never silently creates one; using that reminder menu or its custom date picker keeps the attached Quick Add preview open until the person finishes or deliberately dismisses it. Pasting only a web link immediately attaches the link and proposes an editable title; when safe public metadata is available, the proposal becomes page-specific without delaying capture or overwriting a user edit. Continuing to compose the same Quick Add query—such as typing another character or adding a tag—preserves an edited preview title, resolved link metadata, and the selected reminder instead of treating each keystroke as a new task. Once Detected details appears, its rectangle stays spatially stable while typing and updates its contents in place; a partial unparsable value shows an updating state instead of making the whole editor disappear. Dates, recurrence, duration, tags, links, notes, and other details can be added when useful. A longer thought can open the full editor without retyping the title. Task count never blocks capture or redirects the person into a purchase flow.
...
**Evidence:** User-reported capture failure on 2026-08-20. Refined by user-provided Mac toolbar and workspace-menu screenshots on 2026-08-27 requesting one `+` menu for Add New Task and Focus with visible shortcuts. User-provided iOS first-run feedback on 2026-08-30 requested that Home ask for the first task before showing empty advanced workspaces and explicitly approved revising the earlier empty-Home behavior.

**Navigation evidence:** On 2026-08-29 the user explicitly requested and approved the revised iOS navigation: New contains Create Task followed by Focus, Timeline is replaced by Stats in the tab bar, and More is replaced by Settings.
...
**iOS navigation expectation:** Home, Search, New, Stats, and Settings are stable primary tabs. Timeline remains the history row at the end of Home rather than consuming a tab, so Backlog, Timeline, and Task Ladder still form one predictable workspace group below the task list.

**Desired experience:** Today, planned work, recurring responsibilities, future work, and intentionally hidden or backlogged work have understandable places. Important context is visible without opening every item. On iOS, Home ends with Backlog, Timeline, and Task Ladder rows so deferred work, history, and ranked comparison remain reachable without replacing a bottom tab. They stay out of dedicated Search results and remain below the creation action when an established Home has no tasks. A genuinely new installation temporarily replaces those advanced destinations with the focused first-task experience defined in UC-01. In the Mac main task list, the title gets the complete first line; people who value the full wording of long task names over uniform row height can enable multiline titles in Task List appearance. Status, planning labels, Tags, behavior Flags, and linked Goals always follow the complete title block so a badge never shortens the task's primary identifier. A task with an unfinished linked prerequisite reads as Blocked in the Home task list and wherever Task Details summarizes its effective state, never simultaneously as To Do, Ready, or In Progress; resolving the prerequisite restores the person's earlier workflow state without rewriting its history. Repeating task chains advance one completion at a time: finishing a prerequisite unlocks the dependent until that dependent finishes, even if the prerequisite immediately recurs or is paused to leave the active list. Optional grouping and filters reduce noise without deleting anything, and compact filter screens name each choice once instead of making the person scan repeated headings. On iOS, dedicated Task Type, One-time State, and Priority filter sheets present single-choice options as full-width grouped rows, matching Group rows and preserving readable labels and tap targets at larger text sizes. Priority keeps Importance, Urgency, Pressure, and Thinking needed independent; threshold choices say `or higher`, while exact-value filters clearly distinguish `All` from `None`. In the Mac Stats sidebar, Importance and Urgency appear as separate filter sections. In both places they remain independently adjustable, so changing one judgment never requires reselecting the other. Choosing filter tags feels like choosing tags while editing a task: one searchable plus/check list keeps each tag in one place, while the filter-only Show/Hide and All/Any choices remain explicit. After returning to Filters, the Filter tags row shows every active tag and grows to fit rather than hiding selections behind truncation or a count. On Mac, Shared Flag filtering follows that same compact searchable pattern rather than exposing or duplicating the full Flag catalog in Task List, Timeline, and Calendar: one edit action opens a combined Include/Exclude picker, chosen Flags remain visible outside it as removable summaries, and each multi-Flag rule exposes its own All/Any choice while that side is being edited. Include is a deliberate way to inspect normally hidden tasks, while Exclude remains the final veto when rules overlap.

On Mac, `Shared` offers every Task Ladder value as an independent cross-surface filter and uses the current values the person sees in Task Ladder. The scope explains that it applies to task-backed Task List, Timeline, and Calendar rows, while active dots keep filters in the other scopes discoverable. Timeline Type and Status remain independently adjustable, so choosing one never resets the other. Shared Tags stays calm when the catalog is large: one edit action opens a combined Include/Exclude searchable popover, selected rules stay visible as removable summaries outside it, and deliberate browse keeps selected tags pinned with bounded suggestions. The Tag editor sits in a titled teal panel, while the equivalent Flag editor sits in a titled orange panel, so each family is visually clear without repeating two large action rows. When neither rule is active, the edit action follows the title directly without redundant `No tag filters` or `No flag filters` copy. Stats uses the same recognizable Flags and Tags panels while keeping its report filters independent from Planner. In the narrow companion pane, a single-choice filter remains segmented when its choices fit one line and becomes one compact menu picker when segments would wrap. Shared Task Ladder and Task List menu rows place the title leading and the value trailing on one line, and every picker uses the same width so both edges stay aligned as selections change. Fullscreen filter editing keeps the controls at a readable centered width and uses that width for full-width, single-row choices whenever they fit. Task List puts its three visibility switches before Task type and uses the same left-aligned text, right-aligned switch, and full-row interaction as Appearance, so the card begins with the immediate show/hide choices and scans as one coherent form.

On Mac, one workspace-level control action sits immediately left of the combined workspace menu and appears for Planner, Backlog, and Task Ladder. The menu remains fixed at the trailing edge when the action disappears in other workspaces. Planner keeps its established Shared, Task List, Timeline, and Calendar meanings, while Backlog separates its own controls into `Filter`, `Sort`, and `Appearance`: filtering covers type/status, created date, due-date status, current Task Ladder values, estimate, media, Tags, and Flags; sorting covers due-date order; and Appearance lets the person independently choose multiline titles and the Backlog row context they want to scan. The active-workspace icon makes non-default controls discoverable without adding irrelevant chrome to Stats or other workspaces. Task Ladder uses the same companion/fullscreen pattern with separate `View`, `Sort`, and `Appearance`: metric, Base/Now, group creation, refresh, direction, and count move out of its list-level bar, while independent durable Appearance lets the person choose row density without changing Main Task List or Backlog rows.

Feature availability remains honest on iOS and Mac: when Goals is disabled, Goal creation, filters, deep links, Settings language, linked task-row labels, and Task Detail summaries stay out of the experience without deleting stored Goals or links, and development re-enablement restores them.
...
**Desired experience:** Settings search matches both destination names and user-facing controls that actually exist on the current platform. A result names the matching control or behavior instead of advertising a Mac-only choice on iOS. Selecting the result opens the same destination and leaves its controls unchanged.

**Successful outcome:** The person knows why a category matched and can reach the relevant control without guessing.

**Example:** Searching `hide` returns `Flags` with four hide behaviors on Mac and the three iOS-relevant hide behaviors on iPhone. Searching `Planner calendar`, `Calendar list`, or `keyboard` finds the relevant Mac destination but does not create a false iOS result. Searching `backup` can return `iCloud & Backup` with its sync, export, import, or backup concepts.
...
**Evidence:** User-reported first-edit failure with supplied macOS images on 2026-08-26 established that an existing container group's identity must load with its metric choices and that a metric-only change must remain saveable. User-supplied Mac Task Ladder images on 2026-08-26 established that changing the selected group must not leave selection tint on other lazy rows, then showed that the root workspace title, sort direction, group terminology, and read-only section meaning were repeated across adjacent header layers and should be stated once. User feedback on 2026-08-28 established that iOS inner ladders must participate in native push navigation so Back restores the preceding Ladder list instead of returning directly to Home. A follow-up user-supplied iOS image on 2026-08-29 showed that interactive Back from Group Details still revealed Home underneath instead of the Task Ladder list. A follow-up Mac image on 2026-08-29 showed that the first lazy-row safeguard was insufficient for ordinary task-to-task selection: the new task's details opened while the previous task retained the selection tint. Further Mac feedback the same day isolated the remaining transition: carrying a selected row from Pressure into Urgency could make the next selection leave the old tint, omit the new tint, or show both. User feedback on 2026-09-04 explicitly approved superseding the compact Task Ladder control bar: every sort, setting, count, group, and refresh control moves to the right-side surface, which also gains independent row appearance.

### UC-06B — Let due work become timely without competing too early

**Situation:** A task may be possible long before its due date but irrelevant to today's comparison. A repeating responsibility can additionally matter differently across each recurrence cycle.

**Need:** Describe when any task with a real due boundary should enter Task Ladder, and let repeating Due work change its values across the cycle without remaining artificially urgent after completion.
...
Any task with a concrete due boundary can separately choose when it enters Task Ladder: throughout its active lifetime or recurrence cycle, a chosen number of days before due, or only on the due date. A concrete boundary means a one-time task's deadline or a repeating Due routine's next occurrence; an availability-only one-time task, Gentle routine, or cadence-free routine has no such control. Throughout remains the default because a due date normally describes the last acceptable time, not the first possible time. Before the chosen window, Task Ladder omits the task from every ranking metric; explicit search can still recover it and explain when it will enter. The task returns automatically when the window begins. This attention choice stays distinct from repeating-only `Changes over time`: the entry window controls whether the task belongs in the Ladder yet, while temporal values control how a supported repeating task ranks after it belongs there. It does not prohibit completion outside Task Ladder. If people need a strict “cannot be done yet” rule across completion and other actionable surfaces, that is a separate availability use case requiring further validation.
...
User feedback with a supplied Mac Task Ladder image on 2026-08-29 established a separate need: a short task due much later should not compete in the Ladder before it is actually timely. The image also showed why low `Changes over time` values are insufficient when the Ladder is sorted by another metric such as shortest Estimated time. Follow-up direction accepted an independent entry-window control while preserving completion behavior, then explicitly clarified that the control belongs to every task with a due date rather than repeating tasks alone.

**Availability:** Development experiment on iOS and macOS for one-time tasks with deadlines and repeating Due routines.
...
**Desired experience:** Pausing, archiving, backlog organization, and visibility rules have distinct meanings. Backlog is reachable from the end of iOS Home and as a labeled, full-size workspace in the main Mac window rather than a detached window or hidden app-menu command. Both platforms show the same understandable super-section plus one-level subsection organization, keep deliberately created empty sections visible during normal browsing, and let each level collapse without hiding search matches. Active search or filtering shows only hierarchy that contains a matching task; clearing it restores intentionally empty destinations. Collapsed sections stay collapsed while the person leaves and returns; search expansion never overwrites those choices. Opening Add Task on Mac is a reversible detour: Cancel returns to the Backlog workspace instead of redirecting the person to Planner. A task's `Move to > Backlog` menu keeps all Backlog destinations behind one nested menu and can create a new Backlog super section at the point of use, assigning the task that started the action; Settings remains available for preparing an empty catalog. In Mac Settings -> Sections, a `Main task list` / `Backlog` segmented picker makes the destination of each section obvious and keeps section creation and top-level ordering in the selected surface. Section names and automatic tag rules are independent per destination, so a concept such as `Health` can organize each surface without a misleading duplicate-name error or one surface blocking the other's rule. A tagged Backlog super section automatically gathers matching active tasks hidden from normal task lists even when they retain a Main task list path; that path is not overwritten and returns when the hiding Flag is removed. An explicit Backlog path remains stronger within Backlog, and ordinary tasks without the hiding behavior do not move into Backlog unexpectedly. On iOS, the person can open Task Details and move an explicitly backlogged task to Home from the compact list. The item is not lost, completed, or silently rescheduled. Configuring Flag behavior stays progressive: each Flag shows only the rules already attached to it, `Add Rule` lists the remaining behaviors, and every attached rule can be removed in place.

On Mac, Backlog can be narrowed, ordered, or visually tuned without reorganizing it. Its independent right-side surface separates `Filter`, `Sort`, and `Appearance`: filtering covers task type and status, created date, All/Has Due Date/Due Today/Overdue, current Importance/Urgency/Pressure, Thinking needed, estimate and media presence, Tags, and Flags; sorting covers Default/Due Soonest/Due Latest; and Appearance lets the person choose multiline titles and relevant row fields without changing the Main Task List. Due sorting uses the task's real due boundary—its one-time deadline or a repeating Due routine's next occurrence—and leaves undated or Gentle work after dated work in either direction. Due filtering uses that same true boundary, excludes Gentle and undated work, and updates at the next local day boundary while a day-relative choice is active. It changes row order within each existing Backlog group, not section order or task placement; sorting alone also keeps empty destinations reachable. Filtering still hides empty super sections and subsections so every visible branch explains at least one matching task. Appearance starts with the established sparse multiline row, persists independently, and never hides structural path or pinned context. None of these controls moves a task or changes its path, and a filtered-out search match still prevents accidental duplicate creation.

The Mac Backlog sidebar starts directly with the content a person came to review instead of repeating the active workspace title and explanation above it. Task count and manual refresh remain available in the right-side Backlog controls without permanently reducing the list's height. Those controls lead with the three tabs instead of another Backlog title and subtitle. Each selected tab offers its own clearly scoped reset, so restoring Filter, Sort, or Appearance never discards choices in the other two.
...
On Mac, entering Calendar List closes the focused Day Tasks sidebar and keeps that duplicate agenda unavailable until the person returns to Schedule and deliberately selects a day again. Filters, date selection, and task-detail companions remain available in List.

Planner's filter entry belongs beside the top-right workspace menu rather than inside the already dense planning header. It remains available in both Calendar and Timeline views, opens the same Shared, Task List, Timeline, and Calendar companion/fullscreen scopes, and does not appear in unrelated workspaces.
...
**Evidence:** User-reported Focus, time, and estimate confusion and explicit correction request on 2026-08-24. Refined by user feedback on 2026-08-25 that the active iOS Home timer appeared non-actionable and needed cross-device Pause/Resume, Finish, and Abandon controls. Refined again by user-provided Mac toolbar and workspace-menu screenshots on 2026-08-27 requesting Focus inside the global `+` menu with a visible keyboard shortcut, then by direct feedback that an unexpectedly disabled Focus row and the detached `+` trigger were unclear. Further user screenshots and feedback on 2026-08-27 rejected the labeled replacement controls, requested the earlier compact menu shapes, required the live timer beside the sidebar toggle, and asked the open `+` menu to explain a competing timer. The same user then clarified that merging meant one list: Add New Task and Focus above a divider, followed by Planner, Backlog, Task Ladder, and the other destinations. User-provided empty iOS Focus feedback on 2026-08-30 requested that the missing-task message itself offer a direct route to task creation.
...
**Desired experience:** Outcomes are distinct, the relevant occurrence is clear, and late entry or correction is possible where the meaning remains unambiguous. An unresolved scheduled occurrence is presented as `Needs review`, not ordinary overdue pressure, with direct choices for `It happened`, `Missed`, and `Canceled` and a visible next occurrence. A notification action resolves the occurrence that produced the alert rather than whichever recurrence is current later. An assumption never looks identical to a confirmed completion. In full Mac Task Details, selecting an assumed calendar day makes both factual alternatives available in the joined lifecycle control: the prominent action confirms only that selected date, while `Missed` in the adjacent menu rejects only that date without changing other assumptions. A counted bulk-confirm action remains available as a deliberate secondary menu choice and never replaces the action for Today or another selected date. On Mac, a completed tag Focus block can be corrected in context from Calendar `Schedule`, with its Focus history and calendar evidence staying aligned. If Focus continues across midnight, Timeline and Stats show the actual focused portion on each date. Paused gaps and a later press of Finish never look like focused work.
...
**Desired experience:** Each record keeps its own meaning and appears in a coherent personal history. Optional context features do not complicate the core task experience when unavailable or disabled, including by leaving unavailable choices in creation, Planner, history filters, Timeline rows, Stats reports, task relationships, deep links, Settings language, or notifications. Preserved records remain stored for later re-enablement without advertising a feature that is absent from production. On iOS, disabling Sleep or its shake shortcut means a physical shake cannot open or complete a Sleep-mode start, and preserved Sleep sessions do not remain visible in Timeline while Sleep is unavailable. The current iPhone and Mac releases similarly omit Routina's standalone Event feature everywhere while it is disabled; Apple Calendar review remains available because it imports external calendar entries as tasks rather than creating Event records.
...
**Evidence:** User-provided first-release iOS Timeline feedback on 2026-08-31 established that standalone Event is disabled by default and unavailable to users, so preserved or fixture-generated Events must not appear in release-facing iOS surfaces. Follow-up Mac screenshot feedback on 2026-09-01 established the same expectation for Planner, Timeline, and creation paths in the current Mac release.
...
**iOS global access:** Stats is a direct primary tab rather than a destination nested under More.

**Desired experience:** Stats clearly separates current General Stats from Date Range Stats, so changing the range never appears to affect values such as open One-time tasks or the current Repeating-task inventory. Done, Canceled, and Missed remain selected-period outcomes. Date-bound reports use the same selected boundaries, empty or unavailable reports stay hidden, and a one-day range does not render a trend chart for created tasks. When no report is available yet, the empty-state explanation names only activities the person can currently use; in particular, it does not mention Sleep while Sleep is disabled. On iOS, toolbar actions also appear only when they can affect something: Cards/Compact needs a visible summary, Edit needs a report that can be shown, hidden, or moved, and Filter needs task data or an active filter to clear. A genuinely empty dashboard therefore keeps those three controls out of the way without hiding a real recovery path. Stats distinguishes recorded from assumed activity, counts synchronized copies of one focus session only once, and assigns Focus to the local days and hours in which active work occurred rather than to the day Finish was pressed. It lets the person customize what matters. The first iPhone release stays focused on Routina-owned activity and does not present an Apple Health connection or request Health permission; iPad itself is outside the current release scope. On Mac, a person can click a task-backed summary rectangle such as `Assumed done 32` and inspect an anchored list of the tasks that explain it without leaving Stats. Repeated outcomes stay grouped by task with their occurrence counts, time summaries expose contributing durations, and the list preserves the rectangle's current filters and date meaning. Time charts use the available width on each device and keep date axes sparse, complete, and readable instead of truncating labels or compressing the plot into unused space. On Mac, the sidebar keeps Scope, Show, Time Range, Importance, and Urgency on compact single rows inside passive colored cards. Each current value is a native menu picker, so choosing a filter never expands its option list into the sidebar or moves the controls below it. Only Custom Range reveals additional fields because the person must choose two inclusive dates. Flags and Tags use the same titled, always-expanded panels as Planner Shared, with one combined Include/Exclude searchable popover and visible active-rule summaries, while Stats keeps those selections scoped only to its reports.

**Compact chart refinement:** A cumulative overview shows the whole selected-period trend without requiring horizontal discovery, while the 24-hour rhythm uses structured rows and columns instead of three narrow equal pills that wrap labels and values.
...
**Example:** Open the Time Range menu, choose Month, and immediately scan the newly scoped reports without the sidebar changing height. Click Assumed done to see each contributing task and its assumed-occurrence count in a popover anchored to that rectangle. For a project review, choose Custom and edit both inclusive dates directly below that row.

**Evidence:** Refined by user-provided Mac Stats and Planner screenshots on 2026-08-27, then revised by follow-up feedback that expanding single-choice segments added effort and sidebar movement without benefit. The same feedback requested inline menu pickers for all five single-choice filters. Related follow-up established Repeating/One-time wording, removal of the one-day created-task chart, and a clear general-versus-date-range split while keeping Missed period-scoped. Further user feedback requested that Mac Stats Flags and Tags use Planner Shared's matching filter interaction while preserving Stats as a separate analytical scope. A subsequent annotated Stats image requested that `Assumed done 32` and the other rectangles open anchored task lists. User feedback on 2026-08-30 removed Apple Health and any health-library dependency from the first release. A user-provided iOS empty-Stats image on 2026-08-30 established that empty-state guidance must not mention Sleep while Sleep is disabled. Follow-up empty and edit-mode Stats images on 2026-08-30 established that Edit, Filter, and Cards/Compact must disappear independently whenever they have nothing to affect.

**Release-review evidence:** User-provided iOS images on 2026-08-31 established that cumulative Focus must not look empty when totals exist and that the 24-hour rhythm's three facts need a compact structured layout rather than wrapped pills.

**Availability:** Production for core reports on iPhone and Mac; optional reports vary by platform, data, and feature gate. Apple Health is unavailable in the first iPhone release, and iPad support is deferred.

### UC-14 — Change the system as life changes

Settings search should take the person directly to platform-relevant destinations such as Flags, Tags, Sections on Mac, or iCloud & Backup without searching personal task content. Behavior-bearing built-in Flags remain visible and editable where their behavior can be observed, while ordinary personal labels use Tags.
...
**Additional evidence:** User-provided Flags feedback on 2026-08-30 established that migration guidance is inappropriate because the configurable model has no released users to migrate. Follow-up Tags feedback established that Tag Counter settings are meaningless before any Tag exists and should remain hidden until then.
...
**Successful outcome:** The person can recover data, proceed with an informed reset, or import production data into development with a verified package and a retained destination snapshot instead of using the only live copy as the first test. If a problem is noticed later while Routina remains installed, the person can return to one of the ten newest pre-restore states. Moving devices or reinstalling relies on the verified export saved outside the app; selective record recovery is not currently provided.

**Example:** Before deleting Routina from an old device, the person exports a backup; Routina restores that package in isolation, compares it with the still-available source data, and embeds the resulting manifest and attachment hashes plus semantic fingerprint in a portable receipt. The new device checks the copied file against that receipt and repeats the isolated restore without needing any old data locally, catching damage introduced while copying or storing it. Before resetting cloud data, Routina likewise requires a recent verified export and fresh authentication. Before any restore, Routina preserves the destination state, validates the candidate in isolation, and only then commits the replacement. If the person discovers a missing attachment or history entry weeks later while the app remains installed, a retained pre-restore version remains available for full recovery. Before loading a production export into Dev, the release owner can also run `script/audit_backup.sh` and receive record counts, attachment size, a semantic fingerprint, and confirmation that the portable receipt and isolated restore rounds agree. `Verify Backup` can separately show whether a package exactly matches the data currently open in Routina; it is not a CloudKit server comparison.
...
**Evidence:** User request on 2026-08-30 to revise the outdated Mac screenshot seed for expanded feature coverage and seed the iOS development app for its first release screenshots. Follow-up user-provided iOS Stats and Timeline images on 2026-08-30 established that the fixture must not expose Emotion data because production cannot log Emotions. Additional iOS Timeline feedback on 2026-08-31 established that preserved fixture Sleep sessions must likewise stay out of the release story while Sleep is disabled, and that standalone Events are not available to users in this release and therefore must not be manufactured by the shared fixture. Mac screenshot feedback on 2026-09-01 further established that preserved standalone Events must not leak into Planner or Timeline when the Mac Event experiment is off, and that disabled fixture Goals must not appear in task rows or Task Details when users cannot create them in this version.
