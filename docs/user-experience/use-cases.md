...
The person can also separately choose when each occurrence enters Task Ladder: throughout the recurrence cycle, a chosen number of days before due, or only on the due date. Throughout the cycle remains the default because a due date normally describes the last acceptable time, not the first possible time. Before the chosen window, Task Ladder omits the task from every ranking metric; explicit search can still recover it and explain when it will enter. The task returns automatically when the window begins. This attention choice stays distinct from `Changes over time`: the entry window controls whether the task belongs in the Ladder yet, while temporal values control how it ranks after it belongs there. It also preserves the existing ability to finish an untimed scheduled routine early. If people need a strict “cannot be done yet” rule across completion and other actionable surfaces, that is a separate recurring-availability use case requiring further validation.
...
User feedback with a supplied Mac Task Ladder image on 2026-08-29 established a separate need: a short repeating task due much later should not compete in the Ladder before it is actually timely. The image also showed why low `Changes over time` values are insufficient when the Ladder is sorted by another metric such as shortest Estimated time. Follow-up direction accepted an independent entry-window control while preserving early completion.
