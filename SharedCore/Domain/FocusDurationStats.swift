    ...
    static func points(
        ...
        canonicalSessions.taskSessions.forEach { session in
            ...
            for (day, slices) in slicesByDay where day >= startDate && day <= endDate {
                ...
                var accumulator =
                    contributionsByDay[day, default: [:]][key]
        ...
        canonicalSessions.sprintSessions.forEach { session in
            ...
            let title =
                sprintTitlesByID[session.sprintID].flatMap { $0.isEmpty ? nil : $0 }
                ?? "Board focus"
    ...
    static func groupedPoints(
        ...
        for point in points {
            ...
            for contribution in point.contributions {
                ...
                var accumulator =
                    contributionsByBucket[bucketStart, default: [:]][key]
